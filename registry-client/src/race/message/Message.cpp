...
#include "race/RaceUtil.h"  // rtah::createRandomString, rtah::tokenizeString
...
std::vector<Message> Message::parseSendMessage(const nlohmann::json &payload) {
    try {
        ...
        std::string bypassRoute = payload.at("bypass-route");
        bool isNMBypass = not bypassRoute.empty();
...
std::vector<Message> Message::parseAutoMessage(const nlohmann::json &payload) {
    try {
        ...
        std::string bypassRoute = payload.at("nm-bypass-route");
        bool isNMBypass = not bypassRoute.empty();
        ...
        std::vector<Message> messages(
            count, Message(message, recipient, now, generated, isNMBypass, bypassRoute));
...
std::vector<Message> Message::parseTestPlanMessage(const nlohmann::json &payload) {
    try {
        ...
        std::string bypassRoute = testPlanJson.value("nm-bypass-route", "");
        bool isNMBypass = not bypassRoute.empty();
