...
def attach_node(
    node: str, committees: List[Set[str]], kcomps: List[Set[str]]
) -> List[Set[str]]:
    """
    Purpose:
...
def kcomp_draft(orig_graph: nx.DiGraph, desired_size: int):
    ...
    for node in undrafted_graph.nodes:
        committees = attach_node(node, committees, kcomps)

    return committees


def analyze_committees(committees: List[Set[str]]):
    """
    Purpose:
