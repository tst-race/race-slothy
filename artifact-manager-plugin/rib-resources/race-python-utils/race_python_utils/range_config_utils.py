...
def get_service_from_range_config(
    ...
    for race_service in range_config["range"]["services"]:
        ...
        if race_service["type"] != service_name:
            continue
        ...
        for service_access in race_service["access"]:
            ...
            if ":" in service_url:
                ...
                service_port = general_utils.guess_port_by_protocol(service_protocol)
...
def validate_range_config(
    range_config: Dict[str, Any], allow_no_clients: bool = False
) -> None:
    """
    Purpose:
    ...
    if not get_client_details_from_range_config(range_config) and not allow_no_clients:
        raise Exception(f"No clients found in range config")
    if not get_server_details_from_range_config(range_config):
        raise Exception(f"No servers found in range config")

    # Validate Enclaves (existance)
    if not range_config["range"].get("enclaves"):
        raise Exception(f"No enclaves found in range config")
